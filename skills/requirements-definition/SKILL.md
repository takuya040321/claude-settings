...
4. **技術スタック決定** - 技術選定
5. **アーキテクチャ設計** - レイヤー設計、ディレクトリ構成、設計パターンを決定
6. **要件の整理と確認** - 優先度付けと最終確認
...
以下の観点でアーキテクチャのレイヤー構成をヒアリング：

| カテゴリ | 質問例 |
|---------|--------|
| **アーキテクチャパターン** | クリーンアーキテクチャ、レイヤードアーキテクチャ、ヘキサゴナルアーキテクチャなど、どのパターンを採用しますか？ |
| **レイヤー構成** | プレゼンテーション層・アプリケーション層・ドメイン層・インフラ層の分離方針は？各層の責務と依存方向は？ |
| **ドメインモデル** | 主要なドメインエンティティは何ですか？集約（Aggregate）の単位はどう分けますか？ |
| **データアクセス** | リポジトリパターンを使いますか？ORMの選定は？クエリとコマンドの分離（CQRS）は必要ですか？ |
...
| カテゴリ | 質問例 |
|---------|--------|
| **構成方針** | 機能別（feature-based）とレイヤー別（layer-based）のどちらを基本にしますか？ |
| **モジュール分割** | モノレポ構成ですか？パッケージの分割粒度は？共通コードの配置場所は？ |
| **命名規約** | ファイル・ディレクトリの命名規約は？（camelCase, kebab-case, PascalCase等） |
| **コロケーション** | テストファイル、スタイルファイル、型定義ファイルはソースと同じ場所に配置しますか？それとも別ディレクトリですか？ |

### 5.3 設計パターンと横断的関心事

| カテゴリ | 質問例 |
|---------|--------|
| **エラーハンドリング** | エラーの分類方針は？カスタムエラークラスを定義しますか？エラーの伝搬方法は？ |
| **ロギング** | ログレベルの方針は？構造化ログを使いますか？ |
| **バリデーション** | 入力検証はどの層で行いますか？バリデーションライブラリの選定は？ |
| **DI（依存性注入）** | DIコンテナを使いますか？DIの方針は？ |
| **状態管理** | サーバーサイド：セッション管理の方針は？クライアントサイド：グローバル状態とローカル状態の分離方針は？ |
| **API設計規約** | エンドポイントの命名規約は？レスポンス形式の統一は？バージョニング方針は？ |
