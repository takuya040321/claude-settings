...
### ステップ6：スキルレジストリへの登録（必須）

**重要：** 新しいスキルを作成したら、必ず `skills_registry.json` に登録する。これにより `auto-skill-selector` がスキルを自動推奨できるようになる。

#### レジストリファイルの場所

```
~/.claude/skills/auto-skill-selector/references/skills_registry.json
```
...
```json
{
  "name": "スキル名（SKILL.mdのnameと一致）",
  "description": "スキルの簡潔な説明（日本語）",
  "keywords": ["トリガーキーワード1", "キーワード2", "日本語キーワード"],
  "tech_stack": ["関連技術1", "関連技術2"],
  "file_patterns": ["関連ファイルパターン（例：.py, *.test.ts）"]
}
```

#### 登録手順

1. `skills_registry.json` を読み込む
2. `skills` 配列の末尾に新しいスキルのエントリを追加
3. ファイルを保存
...
**注意事項：**
- `keywords` にはユーザーが使いそうな日本語・英語両方のキーワードを含める
- `tech_stack` は空配列 `[]` でも可
- `file_patterns` は空配列 `[]` でも可
- 登録を忘れると、`auto-skill-selector` がスキルを推奨できない
